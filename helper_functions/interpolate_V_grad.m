function h = interpolate_V_grad(h,i)
...
if isempty(h.closest_point_idx)
    ...
    h.V(i) = h.V_matrix(h.closest_point_idx);

    h.p(i,1) = h.grad{1}(h.closest_point_idx);
    h.p(i,2) = h.grad{2}(h.closest_point_idx);
    h.p(i,3) = h.grad{3}(h.closest_point_idx);
    h.p(i,4) = h.grad{4}(h.closest_point_idx);
    h.p(i,5) = h.grad{5}(h.closest_point_idx);
    h.p(i,6) = h.grad{6}(h.closest_point_idx);
