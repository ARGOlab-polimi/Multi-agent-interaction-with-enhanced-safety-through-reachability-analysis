function ocp_solver = setEgoReference(ocp_solver,model,x0_ego,N,T,h,i,change_reference)
...
if change_reference && limitCount >= 2
    if ego_ontheright(h{1}.simX_0(1:2), h{2}.simX_0(1:2), x0_ego(1:2)) ...
        ...
        disp([num2str(i),' - Emergency merge'])

    elseif x0_ego(3) > 0.02 %x0_ego(2) > 0 ||
        emergency_follow = true;
        disp([num2str(i),' - Emergency follow'])
...
elseif emergency_follow
    ...
    disp([num2str(i),' - Change reference control'])
