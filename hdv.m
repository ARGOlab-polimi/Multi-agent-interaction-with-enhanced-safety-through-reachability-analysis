classdef hdv
    properties
        ...
        dist_vector                                                         % hdv distances vector to ego
        g                                                                   % BRT grid
    ...
    methods
        function obj = hdv(x0, BRT_filedata, control_flag, wh_coeff, ...
                Nsim, V_thr, h_constr)
        ...
        function obj = computeRelative( obj,ego_xy, u0, model,i)
            ...
            [obj.rel_states(i,:), obj.rel_dyn(i,:)] = get_relative(ego_xy, u0, obj.simX_0, obj.simU_0, model);
        end
        ...
        function obj = checkLimitsAndInterpolate(obj, i)
            ...
            [obj.limiti(i), obj.closest_point_idx] = check_limits(obj.rel_states(i,:), obj.g);

            % Interpolate of the current point or extract the exact value 
            % of the out of grid point 
            obj = interpolate_V_grad(obj, i);
        end

        function obj = updateBRT(obj, i, n_vehicle, lane_width)
            % Activation of BRT constraint check
            obj = activate_BRT(obj, i, n_vehicle, lane_width);
        end

        function obj = updateParameters(obj,u0,i)
            % Create solver parameters vector
            obj.params = set_params(obj,u0,i);
        end

        function obj = computeHamiltonian(obj, u0_new, i)
            % Compute hamiltonian value
            obj.U_safe(i) = hamiltonian(obj, u0_new);
        end

        function obj = humanSimulate(obj,i, model, dt)
            % Compute optimal disturbance
            obj = human_control(obj,i,model,dt);

            % Integrate human states due to optimal disturbance
            obj = human_integrate(obj,i,dt);
        end

        function obj = getDistances(obj, x0_ego, model,i)
            if obj.constr.add_circ_dist_constr
                % Compute vector of distances
                obj.dist_vector(i,:) = dist_circles(x0_ego, obj, model);
            else
                obj.dist_vector(i,1) =  (x0_ego(1)-obj.simX_0(1))^2 ...
                    + (x0_ego(2)-obj.simX_0(2))^2 - (model.params.D+model.params.D_h)^2;
